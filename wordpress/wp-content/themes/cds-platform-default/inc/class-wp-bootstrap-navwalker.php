...
if (! class_exists('WP_Bootstrap_Navwalker')) {
    ...
    class WP_Bootstrap_Navwalker extends Walker_Nav_Menu
    {
         ...
         */
        public function start_lvl(string &$output, int $depth = 0, ?WP_Nav_Menu_Args $args = null): void
        {
            ...
            // no-dropdown $classes = array( 'dropdown-menu' );
            $classes = [ '' ];
            /**
         ...
         */
        public function start_el(string &$output, WP_Nav_Menu_Item $item, int $depth = 0, ?WP_Nav_Menu_Args $args = null, int $id = 0): void
        {
            if (isset($args->item_spacing) && $args->item_spacing === 'discard') {
                $t = '';
                $n = '';
            ...
            $split_on_spaces = static function ($class) {
            ...
            if ($this->has_children && $depth === 0) {
                ...
                if ($depth > 0) {
                    // no-dropdown $atts['class'] = 'dropdown-item';
                    $atts['class'] = 'nav-link';
                } else {
                    $atts['class'] = 'nav-link';
                }
         ...
         */
        public function add_schema_to_navbar_ul(array $args): array
        {
         ...
         */
        private function separate_linkmods_and_icons_from_classes(array $classes, array &$linkmod_classes, array &$icon_classes, int $depth): array
        {
                ...
                foreach ($linkmod_classes as $link_class) {
                    if (! empty($link_class)) {
                        ...
                        if ($link_class === 'disabled') {
                            ...
                            $atts['href'] = '#';
                            unset($atts['target']);
                        } elseif ($link_class === 'dropdown-header' || $link_class === 'dropdown-divider' || $link_class === 'dropdown-item-text') {
        ...
        {
            $output = '';
            if ($linkmod_type === 'dropdown-header' || $linkmod_type === 'dropdown-item-text') {
                /*
                 * For a header use a span with the .h6 class instead of a real
                 * header tag so that it doesn't confuse screen readers.
                 */
