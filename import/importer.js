...
export default class Importer {
    ...
    static wpPostCreate = ({ body, title, name, createDate, guid }, attempts = 0) => {
      ...
      async function checkIfExists (guid) {
        ...
        let exists = false
        const checkExists = spawn('docker', ['exec', 'cli', '/usr/local/bin/wp', '--path=/var/www/html',
          'db', 'query',
          `select json_object('id', id) from wp_posts where guid like "%${guid}"`])

        // eslint-disable-next-line no-unreachable-loop
        for await (const data of checkExists.stdout) {
      ...
      const create = (body, title, name, createDate, guid) => {
        ...
        const createPost = spawn('docker', ['exec', 'cli', '/usr/local/bin/wp', '--path=/var/www/html',
          'post', 'create',
                `--post_content=${body}`,
                `--post_title=${title}`,
                `--post_name=${name}`,
        ...
        // WP CLI output
        createPost.stdout.on('data', (data) => {
        ...
        // WP CLI Errors
        createPost.stderr.on('data', (data) => {
